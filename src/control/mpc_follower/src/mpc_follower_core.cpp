...
MPCFollower::MPCFollower() : nh_(""), pnh_("~"), tf_listener_(tf_buffer_)
{
  ...
  // pub_steer_vel_ctrl_cmd_2svl = nh_.advertise<lgsvl_msgs::VehicleControlData>("/svl_ctrlcmd", 1);
  sub_ref_path_ =
    pnh_.subscribe("input/reference_trajectory", 1, &MPCFollower::callbackTrajectory, this);

  // sub_ref_path_from_apm = 
  //   pnh_.subscribe("/pathoptimizer2_targetline", 1, &MPCFollower::callbackRefPathfromapm_cart, this);

  // sub_svlvehicle_status_ = pnh_.subscribe("/chassis", 1, &MPCFollower::callbacksvlVehicleStatus, this);
  // sub_pose_svl_ = pnh_.subscribe("/odom",10,&MPCFollower::callbackPose_svl,this);
...
void MPCFollower::timerCallback(const ros::TimerEvent & te)
{
  ...
  // std::cout <<"steer_cmd:"<<ctrl_cmd.steering_angle * 180 /3.14<<'\n';
}
...
MPCFollower::MPCMatrix MPCFollower::generateMPCMatrix(const MPCTrajectory & reference_trajectory)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    const double ref_vx = current_velocity_ptr_->twist.linear.x;
...
void MPCFollower::callbackRefPathfromapm_cart(const apm_msgs::targetline::ConstPtr &msg)
{
   autoware_planning_msgs::Trajectory tra;
   if(!msg->buf.empty()){
    for(unsigned int i = 0 ; i <msg->buf.size();i = i+ 1)
    {
      autoware_planning_msgs::TrajectoryPoint point;
      point.pose.position.x = msg->buf.at(i).x;
      point.pose.position.y = msg->buf.at(i).y;
      point.pose.position.z = 0;
      point.twist.linear.x  = 1;
      tra.points.push_back(point);
    }
      current_trajectory_ptr_ = std::make_shared<autoware_planning_msgs::Trajectory>(tra);
   }
...
enable_yaw_recalculation_  = true;
  /* calculate yaw angle */
  if (enable_yaw_recalculation_) {
    MPCUtils::calcTrajectoryYawFromXY(&mpc_traj_smoothed);
    MPCUtils::convertEulerAngleToMonotonic(&mpc_traj_smoothed.yaw);
...
void MPCFollower::callbackPose_svl(const nav_msgs::Odometry::ConstPtr& msg)
{
  geometry_msgs::PoseStamped ps;
  geometry_msgs::TwistStamped real_speed;
  ps.header = msg->header;
  ps.pose.position.x = msg->pose.pose.position.x;
  ps.pose.position.y = msg->pose.pose.position.y;
  ps.pose.position.z = msg->pose.pose.position.z;
  ps.pose.orientation = msg->pose.pose.orientation;
  current_pose_ptr_ = std::make_shared<geometry_msgs::PoseStamped>(ps);
  real_speed.twist = msg->twist.twist;
  current_velocity_ptr_ = std::make_shared<geometry_msgs::TwistStamped>(real_speed);
};
...
void MPCFollower::callbacksvlVehicleStatus(const lgsvl_msgs::CanBusData &msg)
{
  current_steer_ptr_    = std::make_shared<double>(-msg.steer_pct);
};
...
void MPCFollower::publishCtrlCmd(const autoware_control_msgs::ControlCommand & ctrl_cmd)
{
  ...
  lgsvl_msgs::VehicleControlData cmdsvl;

  cmd.header.frame_id = "base_link";
  cmd.header.stamp = ros::Time::now();
  cmd.control = ctrl_cmd;
  pub_ctrl_cmd_.publish(cmd);
  // cmdsvl.target_wheel_angle = -ctrl_cmd.steering_angle ;
  // pub_steer_vel_ctrl_cmd_2svl.publish(cmdsvl);
