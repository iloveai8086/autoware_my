...
Simulator::Simulator() : nh_(""), pnh_("~"), tf_listener_(tf_buffer_), is_initialized_(false)
{
  ...
  sub_svlpose     = pnh_.subscribe("/odom",10,&Simulator::Callbacksvlpose,this);
  sub_svlchassis     = pnh_.subscribe("/chassis",10,&Simulator::callbacksvlVehicleStatus,this);
...
void Simulator::Callbacksvlpose(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  current_twist_.linear.x =  msg->twist.twist.linear.x;



}

void Simulator::callbacksvlVehicleStatus(const lgsvl_msgs::CanBusData &msg)
{
  current_twist_.angular.z = -msg.steer_pct;
};
